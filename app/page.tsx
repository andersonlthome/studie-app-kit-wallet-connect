...
export default function Home() {
  ...
  console.log({address, isConnected, chainId, status});

  const { walletInfo } = useWalletInfo();
  console.log({walletInfo});

  const { selectedNetworkId, activeChain, loading, open } = useWeb3ModalState();
  console.log({selectedNetworkId, activeChain, loading, open});

  const { data } = useWeb3ModalEvents();
  console.log({data});

  const { walletProvider,walletProviderType } = useWeb3ModalProvider();
  console.log({walletProvider,walletProviderType});
  ...
  const openNetworkModal = () => web3Modal.open({ view: "Networks" });
